    ...
    try:
        ...
        for dt1 in daterange(start_dt, end_dt):
            if dt1.weekday() not in weekdays:  # to print only the weekdates
                ...
                All_date.append(a)
        # TWD HOLIDAYS FILE
        NSEHolidays = pd.read_csv(METADATA_DIR + '\\' + 'holiday.csv')
...
try:
    ...
    if historical_data == 0:
        ...
        start_dt = date(2022,3,16)     # Starting Date in %y,%m,%d Format Not use Zero Padding
        end_dt = date(2022,3,16)       # Ending Date in %y,%-m,%-d Format Not use Zero Padding
        Daily_Snapshot_data()
except:
