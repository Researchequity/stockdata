...
from os import path
import os
from cash_file_path import input_file_path_fno, inc_file_path_fno, last_row_file_path_fno, clean_file_path_fno
...
def download_stream(index, input_path, inc_file_path, last_row_file_path, break_time, clean_file_path):
    ...
    while datetime.now() <= break_time:  # loop will break when time at server exceed 3:31:00 (10:01:00 at server) PM
        try:
            ...
            os.system(
                'end_var=0 ; [ -f {} ] && end_var=$(awk -F, \'{}\' {}) ; start_var=$end_var ; tail -1 {} > {} ; end_var=$(awk -F, \'{}\' {}) ; awk -F, -v a="$start_var" -v b="$end_var" \'{}\' {} > {}'.format(
                    last_row_file_path, "{print$1}", last_row_file_path, input_path, last_row_file_path, "{print$1}",
                    last_row_file_path, "{if($1>a && $1<=b)print$0}", input_path, inc_file_path))

            chunk = pd.read_csv(inc_file_path, header=None, chunksize=50000)  # nrows=100000)
        ...
        if 0 == 0:  # holiday_check(holiday_file_path):
            for j in range(6):
                ...
                break_time = datetime(year=datetime.today().year, day=datetime.today().day,
                                      month=datetime.today().month, hour=16, minute=35, second=00)

                print('thread start')

                thread = threading.Thread(target=download_stream, args=(
                j, input_file_path_fno.format(i), inc_file_path_fno.format(i), last_row_file_path_fno.format(i), break_time,
...
try:
    ...
    LOG_insert("/workspace/dumper/file.log", formatLOG, e, logging.INFO)
