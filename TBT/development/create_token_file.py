...
sec_df[['token', 'Symbol','Series']].to_csv("/home/workspace/development/python_ankit/token_security.csv",index=False)



con_df = pd.read_csv(contract_filepath, sep='|',header=None,skiprows = 1)
# con_df = con_df[(con_df[2] =='FUTSTK')|(con_df[2] =='FUTIDX')]
con_df = con_df[[0,2,3,30,53,8]]
con_df.columns = ['token', 'Series','Symbol','lotsize','companyName','Option']
#con_df = con_df[con_df['companyName'].str[-6:] =='APRFUT']
con_df[['token', 'Series','Symbol','lotsize','companyName','Option']].to_csv("/home/workspace/development/python_ankit/token_contract.csv",index=False)
