...
def get_R3_breakout_in_hv():
    ...
    num_trades.columns = ['date_hv', 'token', 'normean_quantity', 'sum_qty', 'normean_trd', 'row_count', 'mean_price',
                          'o_price', 'BUY', 'repeat', 'r_qty', 'symbol']
    num_trades['date_hv'] = pd.to_datetime(num_trades['date_hv'])
    ...
    df_ohlc_min = df_ohlc_min.sort_values(['date_min']).reset_index(drop=True)
    df_ohlc_min['value_cumsum'] = df_ohlc_min.groupby(['token'])['value'].cumsum(axis=0)
    df_ohlc_min['qty_cumsum'] = df_ohlc_min.groupby(['token'])['qty'].cumsum(axis=0)
    df_ohlc_min['vwap'] = df_ohlc_min['value_cumsum'] // df_ohlc_min['qty_cumsum']
    df_ohlc_min.drop(['value_cumsum', 'qty_cumsum'], axis=1, inplace=True)

    df_ohlc_min['abv_vwap'] = np.where(df_ohlc_min['close'] >= df_ohlc_min['vwap'], 1, 0)
    df_ohlc_min['abv_vwap_count'] = df_ohlc_min.groupby(['token'])['abv_vwap'].cumsum(axis=0)
    ...
    df_ohlc_min_11_max = num_trades_11.groupby(['token']).agg({'high':['max'],'qty':['sum'],'abv_vwap_count':['max']}).reset_index()
    df_ohlc_min_11_max.columns =['token','high','sum_qty','abv_vwap_count_max']

    # get date of high price marked
    df_ohlc_min_11_max = pd.merge(df_ohlc_min_11_max, df_ohlc_min, on=['token','high'], how='inner')

    df_ohlc_min_11_max = df_ohlc_min_11_max.groupby(['token']).agg({'high':['first'], 'date_min':['first'],'sum_qty':['first'],'abv_vwap_count_max':['max']}).reset_index()
    df_ohlc_min_11_max.columns = ['token','high_max_11','date','qty_11','abv_vwap_count']
    ...
    num_trade_11_breakout['p_change'] = round((num_trade_11_breakout['high_max_11'] - num_trade_11_breakout['o_price']) * 100 / (num_trade_11_breakout['o_price']),1)

    stockmetadata = pd.read_csv('/home/workspace/production/python_ankit/StockMetadata.csv')[['token','MarketCap']]
    num_trade_11_breakout = pd.merge(num_trade_11_breakout, stockmetadata, on='token', how='inner')

    #This to restrict number of number call generation
    num_trade_11_breakout = num_trade_11_breakout[num_trade_11_breakout['p_change'] >= 3.0]
    num_trade_11_breakout = num_trade_11_breakout[(num_trade_11_breakout['abv_vwap_count']/105 > 0.6) & (num_trade_11_breakout['qty_11']/num_trade_11_breakout['normean_quantity'] > 10)]
    #(count_abv_vwap / abv_vwap_sum > 0.6 or qty_ratio > 10) and MarketCap != 'LARGE'
    num_trade_11_breakout = num_trade_11_breakout[num_trade_11_breakout['MarketCap'] != 'LARGE']
