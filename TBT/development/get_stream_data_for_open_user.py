...
def main(interval,script_code,input_file_path,order_id):
    ...
    prefix="/home/workspace/aggregate/stream_by_token/metadata"
    stream = "/home/workspace/aggregate/stream_by_token/"
    ...
    sl_stream_dataframe = os.path.join(prefix, "sl_stream_dataframe_{}_{}.csv".format(date, script_code))
    ...
    df_live_stream['Price_Round'] = ((df_live_stream['pricePaisa']/10).round(rounding))*10 # rounding 
    ...
    if not os.path.exists(sl_stream_dataframe):
        df_live_stream_error.to_csv(sl_stream_dataframe,index=False)
    else:
        sl_stream_old=pd.read_csv(sl_stream_dataframe)
        df_live_stream_error=pd.concat([sl_stream_old,df_live_stream_error])
        df_live_stream_error.to_csv(sl_stream_dataframe, header=None, index=False)
    exit()
    # df_live_stream filter on status ='Pending' ---> df_live_stream_groupbyprice group by Price_Round,Token -- > sum new_quantity
    ...
    df_live_stream_pending.sort_values(by=['Price_Round'],inplace=True,ascending=True)
    df_live_stream_pending=df_live_stream_pending.iloc[3:-3]

    
    df_live_stream_pending_all_price = df_live_stream_pending.pivot(index=['Price_Round'], columns=['token'], values=['new_quantity']).reset_index()
    df_live_stream_pending_all_price.columns = ['Price_Round','b_qty','s_qty']
    ...
    df_live_stream_pending_smlr.columns=['token','sellOrder','S1','S2','S3','Q1','Q2','Q3']
    ...
    #sheet_oi_single.range("A1").options().value = df_stream_token_traded_groupby_buy[df_stream_token_traded_groupby_buy['buyquantity'] > 1000].sort_values("buyquantity", ascending=False).to_csv(stream + 'big_quantity_buy_{}_{}.csv'.format(date,script_code))
    #sheet_oi_single.range("E1").options().value = df_stream_token_traded_groupby_sell[df_stream_token_traded_groupby_sell['sellquantity'] > 1000].sort_values("sellquantity", ascending=False).to_csv(stream + 'big_quantity_sell_{}_{}.csv'.format(date,script_code))

    ##### saving the outputs to excel sheet
    #wb.sheets("Mkt_dep_python").range("A3").options(index=False).value = df_live_stream_pending_all_price
    ...
    #wb.sheets("Traded").range("A1").options(index=False).value = final_df_traded
...
if __name__ == '__main__':
    ...
    #     data_excel_file = "F:\\Dumper\\Python_Ankit\\Stream_ankit_FnO.xlsm"
    # if getpass.getuser() == 'VIJITR':
    #     data_excel_file = "F:\\Dumper\\Python_Ankit\\Stream.xlsm"
    #
    ...
    #stream = str(input("Enter stream number: ")) #str(wb.sheets("Input").range('B1').options(index=False).value)
    #token = str(input("Enter token number: ")) #str(wb.sheets("Input").range('B2').options(index=False).value)
    date = ''.join(str(datetime.today().date()).split('-'))

    date = str(sys.argv[3])
    minround = 1 #int(wb.sheets("Input").range('B3').options(index=False).value)

    stream_path = "/home/workspace/dumper/DUMP_" + str(date) + "_07300" + stream + ".csv_Clean"
    drop_path = "/home/workspace/aggregate/stream_by_token/TBT_" + str(date) + "_" + token + ".csv"

    subprocess.call(("grep {} {} > {}").format("," + token + ",", stream_path, drop_path + "_str"), shell=True)
    ...
    subprocess.call("rm {}".format(drop_path + "_str"), shell=True)
