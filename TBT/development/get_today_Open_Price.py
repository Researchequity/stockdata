...
OPENING_DATA_FILE=os.path.join(OPENING_DATA,"openingdata_{}.csv".format(date))
...
while datetime.now() <= datetime(year=datetime.today().year, day=datetime.today().day, month=datetime.today().month, hour=9, minute=25, second=00) or onetime == 0:
    ...
    df3 =pd.DataFrame()
    df4 = pd.DataFrame()
    df1 = pd.read_csv(dumper_file_dir + 'DUMP_' + date + '_073001.csv_Clean', header = None, nrows = 1000000)
    df2 = pd.read_csv(dumper_file_dir + 'DUMP_' + date + '_073002.csv_Clean', header = None, nrows = 1000000)

    print('half done')
    df3 = pd.read_csv(dumper_file_dir + 'DUMP_' + date + '_073003.csv_Clean', header = None, nrows = 1000000)
    df4 = pd.read_csv(dumper_file_dir + 'DUMP_' + date + '_073004.csv_Clean', header = None, nrows = 1000000)
    ...
    open_df = df.merge(dummy_df, on=[8,0] , how='inner')
