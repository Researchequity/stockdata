...
while datetime.now() <= datetime(year=datetime.today().year, day=datetime.today().day, month=datetime.today().month, hour=9, minute=25, second=00) or onetime == 0:
    ...
    df1 = pd.read_csv(dumper_file_dir + 'fno_' + date + '_073002.csv_Clean', header = None, nrows = 1000000)
    df2 = pd.read_csv(dumper_file_dir + 'fno_' + date + '_073006.csv_Clean', header = None, nrows = 1000000)
    # print('half done')
