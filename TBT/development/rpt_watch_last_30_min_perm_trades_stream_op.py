...
# NORMEN_TRD_FILE = 'Y:\production\python_ankit\Average.csv'
...
last_30min_date = (datetime.today() - dt.timedelta(minutes=30))
df_5_groupby = df_5_groupby[df_5_groupby['Date'] >= last_30min_date]
...
# wb.sheets("buy_last_30min").range("A1").options(index=False).value = df_6.sort_values("repeat_bc", ascending=False)
df_6_csv = df_6.sort_values("repeat_bc", ascending=False)
df_6_csv.to_csv(aggregate_file_dir + '//report//watch_last_30_min_perm_trades_buy_last_30min.csv', index=None)
