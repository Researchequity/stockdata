...
def return_result_by_odr_T(df_stream, token):
    ...
    # df_stream = df_stream[df_stream[4] == "51783"]
    filtered_df_stream = df_stream[df_stream[1] == 'T']
    filtered_df_stream[9] = filtered_df_stream[5] * filtered_df_stream[6]
    frame = filtered_df_stream.groupby([7, 4]).agg(
        {0: ['count'], 6: ['sum'], 9: ['sum'], 5: ['first', 'max', 'min', 'last']}).reset_index()
...
def start_thread(df_norman_trd, interval, token, dumper_file, LOG_FILE_NAME):
    ...
    skip_row_path = os.path.join(SKIP_ROWS_NUM_DIRECTORY,
                                 str(str(dumper_file).split('/')[-1]).split('.')[0]) + "_skipROW_{}.csv".format(
        DATE_TODAY)
    LAST_MIN_DATA_FILE = os.path.join(LAST_MIN_DATA, str(str(dumper_file).split('/')[-1]).split('.')[
        0]) + "_lastmindata_{}.csv".format(DATE_TODAY)
    OPENING_DATA_FILE = os.path.join(OPENING_DATA, "openingdata_{}.csv".format(DATE_TODAY))
    print(OPENING_DATA_FILE)
    ...
    if os.path.exists(LAST_MIN_DATA_FILE):
        ...
        last_min_df['Date'] = pd.to_datetime(last_min_df['Date'])
    else:
    ...
    while datetime.now() <= datetime(year=datetime.today().year, day=datetime.today().day, month=datetime.today().month,
                                     hour=15, minute=35, second=00) or onetime == 0:
        onetime = 1
        ...
        try:
            ...
            chunk = pd.read_csv(dumper_file, header=None, chunksize=6000000, skiprows=read_row)

            for df_stream in chunk:

                try:
                    df_stream[7] = pd.to_datetime(df_stream[7]).dt.ceil(
                ...
                if len(df_stream):
                    ...
                    try:
                        ...
                        df_4 = pd.merge(df_1, df_2,
                                        on=['Date', 'token', 'Symbol', 'Series', 'lotsize', 'companyName', 'Option'])

                        df_5 = pd.merge(df_4, df_3,
                                        on=['Date', 'token', 'Symbol', 'Series', 'lotsize', 'companyName', 'Option'])
                        # print(df_5['Date'].min(),df_5['Date'].max())
                    ...
                    # print('last_min_df',last_min_df['Date'].min(), last_min_df['Date'].max())

                    TODAY_DAY = last_min.day
                    TODAY_MONTH = last_min.month
                    TODAY_YEAR = last_min.year

                    if datetime.now() <= datetime(year=datetime.today().year, day=datetime.today().day,
                                                  month=datetime.today().month, hour=9, minute=20,
                                                  second=00) or open_price_read == 0:
                    ...
                    # today_09_25 = datetime(day=TODAY_DAY,month=TODAY_MONTH,year=TODAY_YEAR,hour=9,minute=25,second=0)
                    # open_df = df_5[(df_5['Date'] == today_09_15 )]
                    # open_df[['token','vwap_price']].to_csv(OPENING_DATA_FILE,mode='a',header=None,index=False)

                    if len(last_min_df):
                        last_min_df.to_csv(LAST_MIN_DATA_FILE, index=False)
                    ...
                    if len(df_5):
                        ...
                        df_ohlc = df_5_groupby.agg(
                            {'open': ['first'], 'high': ['max'], 'low': ['min'], 'close': ['last'], '9_sum': ['sum'],
                             'sum_qty': ['sum']}).reset_index()
                        df_ohlc.columns = ['Date', 'token', 'Symbol', 'open', 'high', 'low', 'close', 'value', 'volume']
                        df_ohlc['Date'] = df_ohlc['Date']
                        df_ohlc[['Date', 'token', 'open', 'high', 'low', 'close', 'value', 'volume']].to_csv(
                            os.path.join(TRADE_WATCH, "ohlc_{}.csv".format(DATE_TODAY)), mode='a', index=False,
                            header=False)
                        ...
                        df_5_groupby = df_5_groupby.agg(
                            {'row_count': ['sum'], 'sum_qty': ['sum'], 'nbuy_count': ['sum'], 'nsell_count': ['sum'],
                             'vwap_price': ['mean'], 'nbuy_qty': ['sum'], 'nsell_qty': ['sum']})

                        df_5_groupby.reset_index(inplace=True)

                        df_5_groupby.columns = ["".join(x) for x in df_5_groupby.columns.ravel()]
                        df_5_groupby.rename(columns={"row_countsum": "row_count", "sum_qtysum": "sum_qty",
                                                     "nbuy_countsum": "nbuy_count", "nsell_countsum": "nsell_count",
                                                     "vwap_pricemean": "vwap_price", "nbuy_qtysum": "nbuy_qty",
                                                     "nsell_qtysum": "nsell_qty"}, inplace=True)
                        ...
                        try:
                            ...
                            df_6 = pd.merge(df_5_groupby, df_norman_trd, on=['token', 'Symbol'])

                            df_6 = df_6[["Date", "token", "Symbol", "row_count", "sum_qty", "nbuy_count", "nsell_count",
                                         "vwap_price", "normean_quantity", "normean_trd", "nbuy_qty", "nsell_qty"]]

                            df_6 = df_6[((df_6['sum_qty'] >= 3 * df_6['normean_quantity']) & (
                                    df_6['row_count'] >= 20))]  # | (df_6['sum_qty'] >= 50*df_6['normean_quantity'])
                            ...
                            df_6 = df_6[(df_6['repeat'] >= 0) & (df_6['row_count'] >= 20) & (
                                    df_6['vwap_price'] > df_6['o_price'])]  # & (df_6['normean_trd'] >10)
                            ...
                            if len(df_6):
                                ...
                                df_6[['Date', 'token', 'normean_quantity', 'sum_qty', 'normean_trd', 'row_count',
                                      'vwap_price', 'o_price', 'BUY', 'repeat', 'r_qty', 'Symbol']].to_csv(
                                    os.path.join(TRADE_WATCH, "stream_op_{}.csv".format(DATE_TODAY)), mode='a',
            ...
            if last_min == datetime(year=datetime.today().year, day=datetime.today().day, month=datetime.today().month,
                                    hour=15, minute=29, second=00) and datetime.now() >= datetime(
                year=datetime.today().year, day=datetime.today().day, month=datetime.today().month, hour=15,
                minute=31, second=00):
                df_5 = last_min_df
                df_5['vwap_price'] = df_5['vwap_price'].astype(int)
                df_5_groupby = df_5.groupby(['Date', 'token', 'Symbol'])
                ...
                df_ohlc[['Date', 'token', 'open', 'high', 'low', 'close', 'value', 'volume']].to_csv(
                    os.path.join(TRADE_WATCH, "ohlc_{}.csv".format(DATE_TODAY)), mode='a', index=False, header=False)
                ...
                df_5_groupby.columns = ["".join(x) for x in df_5_groupby.columns.ravel()]
                df_5_groupby.rename(
                    columns={"row_countsum": "row_count", "sum_qtysum": "sum_qty", "nbuy_countsum": "nbuy_count",
                             "nsell_countsum": "nsell_count", "vwap_pric"
                                                              "emean": "vwap_price", "nbuy_qtysum": "nbuy_qty",
                             "nsell_qtysum": "nsell_qty"}, inplace=True)
                df_5_groupby['vwap_price'] = df_5_groupby['vwap_price'].astype(int)
                df_5_groupby.to_csv(resulting_filepath, mode='a', index=False, header=False)  # keep same
                last_min_df = pd.DataFrame()
...
def filtering_stream_data():
    ...
    if interval in [1, 5]:
        ...
        df_norman_trd['normean_quantity'] = df_norman_trd['vol_norm_mean'].apply(
            lambda x: (x // division_factor))  # mean_quantity
        df_norman_trd['normean_trd'] = 100  # df_norman_trd['mean_trd'].apply(lambda x: (x//division_factor))
        df_norman_trd = df_norman_trd[['Stock', 'token', 'normean_quantity', 'normean_trd']]

        token = pd.read_csv(TOKEN_SECURITY_FILE)
        filename = ('fno_{}_*.csv_Clean').format(DATE_TODAY)
        files = glob(os.path.join(DUMPER_FILE_DIRECTORY, filename))
        print(files)
        # files = ['//home/workspace/dumper/fno_20220311_073002.csv_Clean']
