...
def start_thread(df_norman_trd,interval, token,dumper_file,LOG_FILE_NAME):
    ...
    while datetime.now() <= datetime(year=datetime.today().year, day=datetime.today().day, month=datetime.today().month, hour=15, minute=35, second=00) or onetime == 0:
        ...
        try:
            ...
            chunk = pd.read_csv(dumper_file,header=None,chunksize=300000,skiprows=read_row)
            ...
            for df_stream in chunk:
                ...
                if len(df_stream):
                    ...
                    if datetime.now() <= datetime(year=datetime.today().year, day=datetime.today().day, month=datetime.today().month, hour=9, minute=20, second=00) or open_price_read == 0:
                    ...
                    if len(df_5): 
                        ...
                        try:
                            ...
                            df_6 = df_6[((df_6['sum_qty'] >= 3*df_6['normean_quantity']) & (df_6['row_count'] >= 80))] #| (df_6['sum_qty'] >= 50*df_6['normean_quantity'])
                            ...
                            df_6 = df_6[(df_6['repeat'] >= 0) & (df_6['row_count'] >= 80) & (df_6['vwap_price'] > df_6['o_price']) ] #& (df_6['normean_trd'] >10)
...
def filtering_stream_data():
    ...
    if interval in [1,5]:
        ...
        files = glob(os.path.join(DUMPER_FILE_DIRECTORY, 'fno_*.csv_Clean'))
        print(files)
