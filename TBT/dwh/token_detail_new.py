...
def CalculationOfNeedToTradeForAOrder():
    ...
    for orderID in sorted(gOrderDictionary.keys()):
        if gOrderDictionary[orderID].side == 'B':
            buy_list_dick[orderID] = gOrderDictionary[orderID].qty
        else:
            sell_list_dick[orderID] = gOrderDictionary[orderID].qty
    ...
    sell_list = [] 
    order_dick = {}
    d_sorted_buy_value = OrderedDict(sorted(buy_list_dick.items(), key=lambda x: x[1], reverse=True))
    i=1
    l=1
    for k, v in d_sorted_buy_value.items():
        order_dick[l] = k
        buy_list.append(str(l)+ ", " +str(k) + ", " +str(gOrderDictionary[k].price) + ", " +str( gOrderDictionary[k].qty) + ", " +str( gOrderDictionary[k].side)+", ")
        if i == int(gDepth):
            l=l+1
            break
        i = i+1
    ...
    d_sorted_sell_value = OrderedDict(sorted(sell_list_dick.items(), key=lambda x: x[1], reverse=True))
    i=1
    for k, v in d_sorted_sell_value.items():
        order_dick[l] = k
        sell_list.append(str(l)+ ", " +str(k) + ", " +str(gOrderDictionary[k].price) + ", " +str( gOrderDictionary[k].qty) + ", " +str( gOrderDictionary[k].side))
        if i == int(gDepth):
            break
        i = i+1
        l=l+1
...
#           sys.exit()
      ...
      if order_type == 'T' and (new_order != '1'):
        tradedOrder(line)
