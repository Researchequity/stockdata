...
if len(sys.argv) == 7:
  ...
  gToken = sys.argv[2]
  gFilterprice = sys.argv[3]
  gQtyPercentage = sys.argv[4]
  gStartTime = sys.argv[5]
  gEndTime = sys.argv[6]
...
def CalculationOfNeedToTradeForAOrder():
    ...
    print "\nQuantity parcentageOfQty     OrderID             OrderPrice     QrderQuantity"
    if gBuyPendingQty == 0:
        for key in sorted(gOrderDictionary.keys()):
            if int(gSellPendingQty*(float(gQtyPercentage))) < gOrderDictionary[key].qty and gOrderDictionary[key].price == int(gFilterprice):
                print gSellPendingQty, int(gSellPendingQty*(.01)), key, gOrderDictionary[key].price, gOrderDictionary[key].qty

    if gSellPendingQty == 0:
        for key in sorted(gOrderDictionary.keys()):
            if int(gBuyPendingQty*(float(gQtyPercentage))) < gOrderDictionary[key].qty and gOrderDictionary[key].price == int(gFilterprice):
