...
from os import path
import os
from cash_file_path import input_file_path_cash, inc_file_path, last_row_file_path, clean_file_path
...
def download_stream(index,input_path,inc_file_path,last_row_file_path,break_time,clean_file_path):
    print(input_path)
    while datetime.now()<=break_time or 0==0: # loop will break when time at server exceed 3:31:00 (10:01:00 at server) PM
        try:
    ...
    try:
        ...
        if 0==0: #holiday_check(holiday_file_path):
            for j in range(4):
                ...
                del_last_day_file(i)
                print('thread start')
                thread=threading.Thread(target=download_stream,args=(j,input_file_path_cash.format(i),inc_file_path.format(i),last_row_file_path.format(i),break_time,clean_file_path.format(i)))
