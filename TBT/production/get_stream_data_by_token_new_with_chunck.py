...
from cash_file_path import get_stream_by_token_input_file
...
def main(interval,script_code,input_file_path,order_id):
    ...
    df_stream_token_traded_groupby_buy.rename(columns={"quantity": "buyquantity", "pricePaisa": "b_avg_price","datetime": "b_datetime"},
                                                  inplace=True)
    df_stream_token_traded_groupby_sell = final_df_traded.groupby(['sellOrder']).agg({'quantity': sum, 'pricePaisa': ['mean'], 'datetime':['max']})
    df_stream_token_traded_groupby_sell.rename(columns={"quantity": "sellquantity", "pricePaisa": "s_avg_price","datetime": "s_datetime"},
                                                   inplace=True)
    ...
    df_live_stream['Price_Round'] = df_live_stream['pricePaisa'].round(-2) # rounding it to 100 place
...
if __name__ == '__main__':
    ...
    if getpass.getuser() == 'root':
        data_excel_file = "/home/workspace/development/Report/Stream.xlsm"
    if getpass.getuser() == 'VIJITR':
        data_excel_file = "C:\\Python_Ankit\\Stream.xlsm"

    #wb = xw.Book(data_excel_file)
    print(data_excel_file)
    wb=opxl.load_workbook(data_excel_file)
    print(wb,"got workbook")
    exit()
    ...
    #stream_path = "E:\\DUMPER\\DUMP_" + str(date) + "_07300" + stream + ".csv_clean"
    stream_path =get_stream_by_token_input_file.format(1)
