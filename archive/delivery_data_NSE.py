...
if is_trial == 1:
    sleep_dup = 3
    inp_stock_df = inp_stock_df.head(50)
else:
    sleep_dup = 300
...
while tries <= max_retries:
    ...
    if is_date_eod[12:15] == "EOD":
        new_eod_date = is_date_eod[:11] + " 16:00:00"
        dp_df_check['secWiseDelPosDate'] = new_eod_date
...
def start_thread(Stock, session_count,index,s):
    ...
    try:
        ...
        if df_metadata == 'BE':
            url_del = 'https://www.nseindia.com/api/quote-equity?symbol=' + Stock.replace('&',
                                                                                          '%26') + '&section=trade_info'
...
if is_data_dup == 0:
    ...
    #Check date if EOD replace with 4 PM
    delivery_position_all_df = delivery_position_all_df[delivery_position_all_df['secWiseDelPosDate'] == delivery_position_all_df['secWiseDelPosDate'].max()]
    is_date_eod = delivery_position_all_df['secWiseDelPosDate'].max()#.iloc[0]


    if is_date_eod[12:15] == "EOD":
        new_eod_date = is_date_eod[:11] + " 16:00:00"
        delivery_position_all_df['secWiseDelPosDate'] = new_eod_date
    ...
    # delivery_position_all_df[
    #     ['Stock', 'secWiseDelPosDate', 'lastPrice', 'pChange', 'vwap', 'quantityTraded', 'deliveryQuantity',
    #      'deliveryToTradedQuantity']].to_csv("D:\\Program\\Data\\nse_del_data.csv", index=False)
...
if timesleep < "19:55":
    ...
    if not block_deal_df.empty:
        ...
        block_deal_df.to_csv(PROCESSED_DIR + '\\nse_delivery_position.csv', index=False, header=False, mode='a')
