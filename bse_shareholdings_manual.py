...
#                 'Nationality': 'Type', 'Category of shareholder': 'Detail'}, axis=1, inplace=True)
...
if SuperStar == 1:
    ...
    # data_df = data_df[data_df['Detail'].str.contains("JHUNJHUNWALA RAKESH|Jhunjhunwala Rakesh", na=False)]
    ...
    data_base_df = data_base_df[data_base_df['Script_code'].isin(script_code_today)]
    data_base_df['Company'] = data_base_df['Script_code']
    # each quater name are different so just pick one
    # company = data_base_df['Company'].unique()[0]
    # data_df['Company'] = company
    # data_base_df['Company'] = company
...
# Get Header Data
if SuperStar != 1:
    data_header_df = data_df[data_df['Category'] == data_df['Category of shareholder']]
    ...
    data_header_df.drop_duplicates(subset=["Script_code", "Company", "Quarter", "Nationality", "Category", "Header"], keep=False,inplace=True)

    #data_header_df.to_csv(input_filepath + '\\stage_fii_comp.csv')
    data_header_df = data_header_df.pivot(index=['Script_code', 'Company', 'Nationality', 'Category'], columns='Quarter',values=['Total nos. shares held', 'Nos. of shareholders'])
    ...
    print(data_header_csv.head())

    for i in range(len(quarters)-1):
        print(quarters)
        i=i+1
        print(i)
        data_header_csv['diff_share_%_q'+str(i)+'q'+str(i+1)] = round((data_header_csv['Total nos. shares held_q'+str(i)] - data_header_csv['Total nos. shares held_q'+str(i+1)])* 100 / data_header_csv['Total nos. shares held_q'+str(i+1)],2)
...
# Get Detail Data
data_detail_df = data_df[data_df['Category'] != data_df['Category of shareholder']]
...
data_detail_df.drop_duplicates(subset=["Script_code", "Company", "Quarter", "Nationality", "Category of shareholder"],keep=False, inplace=True)
data_detail_df = data_detail_df.pivot(index=['Script_code', 'Company', 'Nationality', 'Category', 'Category of shareholder'], columns='Quarter', values=['Total nos. shares held', 'Nos. of shareholders'])
...
# data_header_df.to_csv("D:\\Program\\Data\\data_header_df.csv")
# data_detail_df.to_csv("D:\\Program\\Data\\data_detail_df.csv")
