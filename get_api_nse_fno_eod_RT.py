...
if not os.path.exists(r'\\192.168.41.190\program\stockdata\processed\option\RT\opt_chain_all_hist_' + sub_string_date + '.csv'):
    ...
    all_stock_df.to_csv(r'\\192.168.41.190\program\stockdata\processed\option\RT\opt_chain_all_hist_' + sub_string_date + '.csv', index=None)
else:
    historical = pd.read_csv(r'\\192.168.41.190\program\stockdata\processed\option\RT\opt_chain_all_hist_' + sub_string_date + '.csv')
    ...
    if historical['date_today'].max() != all_stock_df['date_today'].max():
        ...
        historical.to_csv(r'\\192.168.41.190\program\stockdata\processed\option\RT\opt_chain_all_hist_' + sub_string_date + '.csv', index=None)
